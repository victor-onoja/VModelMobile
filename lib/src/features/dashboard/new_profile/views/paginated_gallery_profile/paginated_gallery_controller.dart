...
final galleryFeedDataProviderXX = StateProvider<GalleryFeedDataModel?>((ref) {
  return null;
});
...
final enableOtherUserPolaroidProviderXX = StateProvider<bool>((ref) {
...
final galleryTypeFilterProviderXX =
    StateProvider.family<AlbumType, String?>((ref, arg) => AlbumType.portfolio);

final filteredGalleryListProviderXX =
    Provider.family<AsyncValue<List<GalleryModel>>, String?>((ref, argument) {
  final filter = ref.watch(galleryTypeFilterProviderXX(argument));
  final temp = ref.watch(galleryProviderXX(argument));
  ...
  final result = stateValues
      .where((element) => element.galleryType == filter && element.hasPosts)
      .toList();

  // if (temp.isLoading || temp.isRefreshing) {
  final galleries = result.map((e) => e.toGalleryModel).toList();
  return AsyncData(galleries);
});

final galleryProviderXX = AsyncNotifierProvider.family<GalleryNotifier,
    List<UserGalleryOnlyModel>, String?>(() => GalleryNotifier());

class GalleryNotifier
    extends FamilyAsyncNotifier<List<UserGalleryOnlyModel>, String?> {
  final _repository = GalleryRepository.instance;

  @override
  Future<List<UserGalleryOnlyModel>> build(arg) async {
    print(
    ...
    //         element.galleryType == AlbumType.polaroid &&
    //         element.postSets.isNotEmpty);
    //     ref.read(enableOtherUserPolaroidProviderXX.notifier).state = sss;
